...
{
    return std::atan2(velocity.y, velocity.x) * 180.f / M_PI; //authored with chatgpt
}
...
{
    sf::Clock clock;
    CustomSprite boids[60];
    SteeringData steeringData[60];
    // CustomSprite sprite;
    for (int i = 0; i < 60; ++i)
    {
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < 60; i++)
        {
            SteeringData others[59]; // Array to hold other boids' data
            int index = 0;
            for (int j = 0; j < 60; j++)
            {
            ...
            steeringData[i].velocity += 100.f * (1.0f * cohesionForce * dt.asSeconds() + 8.0f * alignmentForce * dt.asSeconds() + 150.f * separationForce * dt.asSeconds()); //multiplying the final by 100 because the force is negligible otherwise
