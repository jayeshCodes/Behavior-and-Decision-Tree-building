...
void large_astar(const Graph &graph, const string &start, const string &goal, map<string, pair<double, double> > coordinates) {
    ...
    // Access adjacency list using unordered_map for faster lookups
    const auto &adjacency_list = graph.getAdjacencyList();

    // Initialize distances and previous nodes with the size of the graph
    unordered_map<string, double> dist(adjacency_list.size());
    unordered_map<string, string> prev(adjacency_list.size());
    ...
    // Use unordered_map for priority queue to avoid copying
    priority_queue<pair<double, string>, vector<pair<double, string> >, greater<pair<double, string> > > pq;
...
while (inputFile2 >> source2 >> sX >> sY >> destination2 >> dX >> dY >> distance2)
{
    ...
    large_astar(large_graph, "1", "1000", coordinates2);
    cout<<'\n';
