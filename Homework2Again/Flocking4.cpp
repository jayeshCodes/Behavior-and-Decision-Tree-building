...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < 30; i++)
        {
            ...
            steeringData[i].velocity += 100.f * (0.8f * cohesionForce * dt.asSeconds() + 1.0f * alignmentForce * dt.asSeconds() + 10.f * separationForce * dt.asSeconds()); //multiplying the final by 100 because the force is negligible otherwise
