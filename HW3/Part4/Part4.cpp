...
std::vector<sf::Vector2f> generateSuccessors(int x, int y)
{
    std::vector<sf::Vector2f> successors;
    // Add neighboring cells (up, down, left, right)
    if (isValid(x - 1, y))
        successors.push_back(sf::Vector2f(x - 1, y));
    if (isValid(x, y - 1))
        successors.push_back(sf::Vector2f(x, y - 1));
    if (isValid(x + 1, y))
        successors.push_back(sf::Vector2f(x + 1, y));
    if (isValid(x, y + 1))
        successors.push_back(sf::Vector2f(x, y + 1));
...
std::vector<sf::Vector2f> findPath(sf::Vector2f start, sf::Vector2f end)
{
    ...
    while (!openList.empty())
    {
        ...
        if (current->x == end.x && current->y == end.y)
        {
            ...
            {
                path.push_back(sf::Vector2f(current->x, current->y));
        ...
        // Generate successors
        std::vector<sf::Vector2f> successors = generateSuccessors(current->x, current->y);
...
//                     float x = event.mouseButton.x / RoomSize;
//                     float y = event.mouseButton.y / RoomSize;
